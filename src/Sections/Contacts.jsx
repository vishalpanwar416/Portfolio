...
export default function Contact() {
  ...
  const createWhatsAppMessage = () => {
    // Always create a message if any field has content and there are no validation errors
    if (!isValid || (!formData.name.trim() && !formData.message.trim() && !formData.email.trim())) {
      return null
    }
    
    let message = `Hello Vishal Panwar..!! \n\n`
    ...
    <section id="contact" className="py-20 bg-gradient-to-br from-gray-50 via-white to-gray-50 relative overflow-hidden">
      ...
      <div className="container mx-auto px-3 md:px-8 relative z-10 max-w-full">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="space-y-4">
              {contactInfo.map((contact, index) => (
                ...
                >
                    ...
                    className="block p-4 sm:p-6 bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 hover:border-gray-200"
                  >
                    <div className="flex items-center space-x-4 flex-wrap">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-600 uppercase tracking-wide truncate">{contact.label}</p>
                        <p className="text-gray-800 font-medium group-hover:text-blue-600 transition-colors duration-200 break-words">
                      {contact.value}
                        </p>
                      </div>
          ...
          <motion.div 
            className="bg-white p-4 sm:p-8 rounded-3xl shadow-2xl border border-gray-100"
            initial={{ opacity: 0, x: 30 }}
            whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div className="w-full">
                <motion.a
                  href={`https://wa.me/917500988212${createWhatsAppMessage() ? `?text=${createWhatsAppMessage()}` : ''}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3 sm:py-4 px-4 sm:px-6 rounded-xl font-bold text-base sm:text-lg shadow-lg transition-all duration-300 transform flex items-center justify-center space-x-2 sm:space-x-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 hover:shadow-xl hover:scale-105 cursor-pointer text-white"
